...
if [ $# != 2 ]; then cat << EOM
    ...
    where ENDPOINT is the path to the ElasticSearch endpoint (or http://localhost:9200)
      and INDEX is the name of the endpoint, e.g. 'initializr'
...
sleep 1
curl -XPUT $ENDPOINT/$INDEX/_settings -d @./config/settings.json
sleep 1
echo "\nInstall the mapping - do once per type/mapping file."
curl -XPOST "$ENDPOINT/$INDEX/request/_mapping" -d @./config/request.json
sleep 1
